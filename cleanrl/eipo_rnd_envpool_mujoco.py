...
if __name__ == "__main__":
    ...
    action_rms = RunningMeanStd(shape=(1, envs.action_space.shape[0]))
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # use ICM model to compute the intrinsic reward (normalize everything for ICM)
            icm_obs = (
                (
            ...
            for idx, d in enumerate(done):
                ...
                if d: # make sure it is terminated not truncated
                    avg_returns.append(info["r"][idx])
                    epi_ret = np.average(avg_returns)
                    print(
                        f"global_step={global_step}, episodic_return={info['r'][idx]}, curiosity_reward={np.mean(curiosity_rewards[step].data.cpu().numpy())}"
                    )
